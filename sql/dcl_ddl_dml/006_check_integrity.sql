...
FROM
    ...
    JOIN models        m ON s.model_id = m.model_id                      -- Check valid model
...
SELECT
    ...
    round((m.model_base_price + nvl(opt.sum_options, 0) - nvl(ti.trade_in_appraisal_price, 0)),
          2)                                                                                                        AS expected_total
          ,
    ( s.sale_total_price - ( m.model_base_price + nvl(opt.sum_options, 0) - nvl(ti.trade_in_appraisal_price, 0) ) ) AS diff
FROM
         sales s
    ...
    LEFT JOIN (
        ...
        GROUP BY
            so.sale_id
    )         opt ON s.sale_id = opt.sale_id
    LEFT JOIN trade_ins ti ON s.trade_in_id = ti.trade_in_id
WHERE
    abs(s.sale_total_price -(m.model_base_price + nvl(opt.sum_options, 0) - nvl(ti.trade_in_appraisal_price, 0))) > 1.0;